...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        if (requestConfig.headers["X-Application-ID"].isNullOrEmpty()) {
            if (apiKey["X-Application-ID"] != null) {
                if (apiKeyPrefix["X-Application-ID"] != null) {
                    requestConfig.headers["X-Application-ID"] = apiKeyPrefix["X-Application-ID"]!! + " " + apiKey["X-Application-ID"]!!
                } else {
                    requestConfig.headers["X-Application-ID"] = apiKey["X-Application-ID"]!!
                }
            }
        }
    }
